...
def lambda_handler(dict_event, context):
    ...
    try:
        ...
        if not str_subject:
            raise Exception(f'Não foi possível concluir o processo pois não foi definido o assunto para envio da notificação de finalização de irrigação.')

        str_message = dict_params.get('str_message')
        if not str_message:
            raise Exception(f'Não foi possível concluir o processo pois não foi definida a mensagem para envio da notificação de finalização de irrigação.')
